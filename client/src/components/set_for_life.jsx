...
class SetForLife extends Component {
	constructor(props) {
		super(props);
	...
	async update() {
		let b = await axios.get("http://localhost:5000/api/setforlife");
		//console.log("Here");
		let curData = b.data;
	...
	async deleteOldContent() {
		...
		if (a.value) {
			...
			if (count >= len)
				...
				console.log(ids);
				await axios.post("http://localhost:5000/api/setforlife/delete", {
					ids: ids,
				});
	...
	getPrediction() {
		if (this.state.dataFetched && this.state.predict) {
			...
			var prediction = predictNumbers(conditions, frequencyNum, data, true);
			return prediction;
		} else {
