...
class MonWed extends Component {
	constructor(props) {
		super(props);
	...
	async update() {
		let b = await axios.get("http://localhost:5000/api/monwed");
		//console.log("Here");
		let curData = b.data;
		console.log(curData);
		curData.sort((a, b) => {
	...
	async deleteOldContent() {
		...
		if (a.value) {
			...
			if (count >= len)
				...
				console.log(ids);
				await axios.post("http://localhost:5000/api/monwed/delete", {
					ids: ids,
				});
